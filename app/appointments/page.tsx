  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        ...
        if (cancelled) return;
        setReminders(r.filter((x) => x.kind === "appointment"));
        setProfiles(p);
        setSelected((cur) => {
    ...
    >
      ...
      <section className="mb-14">
        <h2 className="mb-4 font-display text-[18px] tracking-tight text-ink-soft">
          {activeProfile ? `Nova consulta · ${activeProfile.name}` : "Nova consulta"}
        </h2>
        <OneShotForm profileId={selected} defaultKind="appointment" lockKind />
      </section>
      ...
      {mounted ? (
        <section>
          <UpcomingList
            ...
            profiles={profiles}
            hideKindTag
            emptyUpcoming="Nenhuma consulta agendada."
            emptyHistory="Nenhuma consulta no histórico."
          />
        </section>
      ) : null}
    </Shell>
