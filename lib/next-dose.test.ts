...
function mkDeps(opts: {
  ...
  return {
    async getConfig() {
      return { chatId: 12345, timezone: TZ, ...opts.config };
    },
    async listReminders() {
      return opts.reminders ?? [];
    },
    async getLog(_sub, date) {
      return opts.logs?.[date] ?? {};
    },
    async getNotifiedKeys(_sub, date) {
...
describe("computeNextDose", () => {
  it("returns empty when user has no chatId", async () => {
    const deps = mkDeps({ config: { chatId: undefined } });
    const result = await computeNextDose("u1", deps, { now: NOW });
  ...
  it("picks today's future slot as nextAt", async () => {
    ...
    const result = await computeNextDose("u1", mkDeps({ reminders }), { now: NOW });
  ...
  it("returns slot as due when it's within fire-fudge window", async () => {
    ...
    const result = await computeNextDose("u1", mkDeps({ reminders }), { now: NOW });
    expect(result.due).toEqual([{ sub: "u1", reminderId: "m1", time: "08:00" }]);
  ...
  it("includes slot from last 5 minutes in due", async () => {
    ...
    const result = await computeNextDose("u1", mkDeps({ reminders }), { now: NOW });
    expect(result.due).toEqual([{ sub: "u1", reminderId: "m1", time: "07:58" }]);
  });
